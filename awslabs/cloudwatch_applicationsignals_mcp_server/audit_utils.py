...
async def execute_audit_api(input_obj: Dict[str, Any], region: str, banner: str) -> str:
    ...
    for batch_idx, batch_targets in enumerate(target_batches, 1):
        ...
        try:
            ...
            if 'throttl' in error_msg_lower or 'rate' in error_msg_lower or 'TooManyRequestsException' in error_msg:
                ...
                guidance = 'Permission denied. The Lambda role may lack application-signals:ListAuditFindings permission. Do NOT retry.'
            ...
            elif 'Pagination token' in error_msg or 'pagination' in error_msg_lower:
                ...
                guidance = 'API call timed out. Try with fewer targets or a shorter time range.'
            else:
    ...
    # Prepend severity summary line for quick readability
    from .audit_presentation_utils import format_severity_summary_line
    severity_summary = format_severity_summary_line(aggregated_findings)

    return banner + severity_summary + final_observation_text
